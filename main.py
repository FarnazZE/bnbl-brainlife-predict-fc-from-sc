...
with open(labelFilename2, "r") as fd2:
    labelData2 = json.load(fd2)
    labelDataHasHeader = False

for entry in indexData1:
            entryFilename1 = entry["filename"]
            a = np.loadtxt(os.path.join(CSVDirectory1, entryFilename1),delimiter=",")

for entry in indexData2:
            entryFilename2 = entry["filename"]
            b = np.loadtxt(os.path.join(CSVDirectory2, entryFilename2),delimiter=",")
...
for p in range(len(mats)):
    for i in range(n):
        ...
        r[i,p] = stats.pearsonr(x,y)[0]
        

# transform correlation coefficients into variance explained
rsq = r**2

np.savetxt('output/csv/rsq.csv',rsq,delimiter=',')
